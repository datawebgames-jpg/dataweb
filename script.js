/* script.js - correcciones: DOMContentLoaded, modal funcional, redes correctas */
...
/* Ejecutar solo cuando DOM esté cargado */
document.addEventListener("DOMContentLoaded", () => {
  ...
  // si no hay iconCards, mostrar en consola (debug)
  if (!iconCards || iconCards.length === 0) {
    console.warn("No se encontraron .icon-card en el DOM. Verifica index.html");
  }

  // Abrir modal al clickear icono
  ...
  if (sendWaBtn) {
    sendWaBtn.addEventListener("click", () => {
      ...
      const name = document.getElementById("m-nombre").value || "No indicado";
      const email = document.getElementById("m-email").value || "No indicado";
      const wa = document.getElementById("m-wa").value || "No indicado";
  ...
  /* ---------- GEOLOCALIZACION y widgets (clima + noticias) ---------- */
    ...
    try {
      return await new Promise((resolve) => {
        if (navigator.geolocation) {
          ...
          fetch('https://ipapi.co/json/').then(r=>r.json()).then(j=>resolve({ lat: j.latitude, lon: j.longitude, region: j.region, city: j.city })).catch(()=>resolve(DEFAULT_LOC));
        }
      });
